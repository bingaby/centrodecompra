...
app.use(express.urlencoded({ extended: true }));const storage = multer.diskStorage({
...
const upload = multer({
  ...
  try {
    ...
    const produtos = JSON.parse(fs.readFileSync(produtosPath, 'utf-8'));const { page = 1, limit = 10, categoria, loja } = req.query;
...
res.json({
  ...
  try {
    ...
    const produtos = JSON.parse(fs.readFileSync(produtosPath, 'utf-8'));const { nome, descricao, categoria, loja, link, preco } = req.body;
...
res.status(201).json({ produto, message: 'Produto adicionado com sucesso' });  } catch (error) {
    logError(error, req);
  ...
  try {
    ...
    const produtos = JSON.parse(fs.readFileSync(produtosPath, 'utf-8'));const { id } = req.params;
...
res.json({ produto: updatedProduto, message: 'Produto atualizado com sucesso' });  } catch (error) {
    logError(error, req);
...
res.json({ message: 'Produto excluído com sucesso' });  } catch (error) {
    logError(error, req);
