...
let totalProdutos = 1000;// Função para escapar HTML e evitar XSS
...
async function carregarProdutos() {
  ...
  let attempt = 1;  while (attempt <= maxRetries) {
    try {
      ...
      gridProdutos.innerHTML = '';  const response = await fetch(
...
function filtrarProdutos() {
  ...
  const mensagemVazia = document.getElementById('mensagem-vazia');  if (!gridProdutos || !mensagemVazia) {
    ...
    .slice(0, produtosPorPagina);  console.log(Produtos filtrados: ${produtosFiltrados.length});  gridProdutos.innerHTML = '';
...
`;
gridProdutos.appendChild(produtoDiv);  });  console.log(Exibidos ${produtosFiltrados.length} produtos);
...
modal.setAttribute('aria-hidden', 'false');  } catch (error) {
