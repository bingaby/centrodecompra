...
let totalProdutos = 0; // Alterado para 0, será atualizado dinamicamente// Função para normalizar strings (remover acentos e converter para minúsculas)
...
async function carregarProdutos() {
  ...
  let attempt = 1;  while (attempt <= maxRetries) {
    try {
      ...
      gridProdutos.innerHTML = '';  let url = `${API_URL}/api/produtos?page=${currentPage}&limit=${produtosPorPagina}`;
...
function filtrarProdutos() {
  ...
  const mensagemVazia = document.getElementById('mensagem-vazia');  if (!gridProdutos || !mensagemVazia) {
    ...
    });  console.log('Produtos filtrados:', produtosFiltrados);  gridProdutos.innerHTML = '';
  ...
  gridProdutos.style.display = 'grid';  produtosFiltrados.forEach((produto, produtoIndex) => {
    ...
    const carrosselId = carrossel-${produtoIndex}-${produto._id || Date.now()};const produtoDiv = document.createElement('div');
...
function moveCarrossel(carrosselId, direction) {
  ...
  imagens.dataset.index = currentIndex;  dots.forEach((dot, i) => dot.classList.toggle('ativo', i === currentIndex));
}function setCarrosselImage(carrosselId, index) {
  const carrossel = document.getElementById(carrosselId);
  const imagens = carrossel.querySelector('.carrossel-imagens');
  const dots = carrossel.querySelectorAll('.carrossel-dot');  imagens.style.transform = translateX(-${index * 100}%);
  imagens.dataset.index = index;  dots.forEach((dot, i) => dot.classList.toggle('ativo', i === index));
}// Funções do modal
async function openModal(produtoIndex, imageIndex) {
  ...
  const carrosselDots = document.getElementById('modalCarrosselDots');  try {
    currentImages = Array.isArray(produtos[produtoIndex]?.imagens) && produtos[produtoIndex].imagens.length > 0
      ...
      : ['imagens/placeholder.jpg'];
    currentImageIndex = imageIndex;carrosselImagens.innerHTML = currentImages.map(img => `<img src="${img}" alt="Imagem ampliada" onerror="this.src='imagens/placeholder.jpg'">`).join('');
...
modal.style.display = 'flex';  } catch (error) {
  ...
  carrosselImagens.style.transform = translateX(-${currentImageIndex * 100}%);  Array.from(carrosselDots).forEach((dot, i) => dot.classList.toggle('ativo', i === currentImageIndex));
